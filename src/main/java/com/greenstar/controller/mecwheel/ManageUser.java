...
import com.greenstar.dal.Response;
import com.greenstar.entity.crb.User;
...
public class ManageUser {
    ...
    public String getUserInfo(String userId){
        ...
        if(userId!=null && !userId.equals("") && userId.length()<20){
            ...
            List<User> users = (List<User>) HibernateUtil.getDBObjects(query);
    ...
    public String insertUser(String userString){
        ...
        if(user!=null && user.getEmail()!=null && !user.getEmail().equals("")
            ...
            Boolean isSuccessful = HibernateUtil.save(user);
            if(isSuccessful){
                response.setMessage("User Attached");
                response.setStatus(Codes.ALL_OK);
                response.setUser(user);
